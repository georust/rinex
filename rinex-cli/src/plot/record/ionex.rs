...
use plotly::{
    ...
    ImageFormat, Layout, Plot, Scatter, ScatterMapbox,
};
use rinex::{ionex::*, prelude::*};

pub fn plot_tec_map(ctx: &mut Context, borders: ((f32, f32), (f32, f32)), record: &Record) {
    let map_center = (
        (borders.1 .0 - borders.0 .0) / 2.0,
        (borders.1 .1 - borders.0 .1) / 2.0,
    );
    ...
    let grid = ScatterMapbox::new(latitudes, longitudes).marker(
        Marker::new()
            .size(5)
