...
pub fn retain_filters(rnx: &mut Rinex, ops: Vec<(&str, Vec<&str>)>) {
    for (op, args) in ops.iter() {
        if op.eq(&"retain-sv") {
            let filter = args_to_space_vehicules(args.clone());
            rnx
                .retain_space_vehicule_mut(filter);
        } else if op.eq(&"retain-constell") {
            let filter = args_to_constellations(args.clone());
            rnx
                .retain_constellation_mut(filter);
        } else if op.eq(&"retain-epoch-ok") {
            rnx
                .retain_epoch_ok_mut();
        } else if op.eq(&"retain-epoch-nok") {
            rnx
                .retain_epoch_nok_mut();
        } else if op.eq(&"retain-obs") {
            rnx
                .retain_observable_mut(args.clone());
        } else if op.eq(&"retain-ssi") {
        } else if op.eq(&"retain-orb") {
        } else if op.eq(&"retain-lnav") {
            rnx
                .retain_legacy_navigation_mut();
        } else if op.eq(&"retain-mnav") {
            rnx
                .retain_modern_navigation_mut();
        } else if op.eq(&"retain-nav-msg") {
            let filter = args_to_nav_message(args.clone());
            rnx
                .retain_navigation_message_mut(filter);
        
        } else if op.eq(&"retain-nav-eph") {
            rnx
                .retain_navigation_ephemeris_mut();
        } else if op.eq(&"retain-nav-iono") {
            rnx
                .retain_navigation_ionospheric_models_mut();
        }
    }
}
