    ...
    if let Some(nav) = nav {
        ...
        if let Some(obs_rec) = rnx.record.as_obs() {
            if let Some(nav_rec) = nav.record.as_mut_nav() {
                ...
                let obs_epochs: Vec<Epoch> = obs_rec.keys().map(|(k, _)| *k).collect();
                ...
                let nav_epochs: Vec<Epoch> = nav_rec.keys().map(|k| *k).collect();
                ...
                for (_, (_, vehicules)) in obs_rec {
                    for (sv, _) in vehicules.iter() {
                        if cmap.get(sv).is_none() {
                            cmap.insert(*sv, Palette99::pick(sv.prn.into()).mix(0.99));
                        }
                        if sv.prn > max_prn {
                            max_prn = sv.prn;
                        }
                ...
                for (sv, color) in &cmap {
                    ...
                    chart
                        .draw_series(
                            data.iter()
                                .map(|point| Cross::new(*point, 4, color.clone())),
                        )
                        .expect("failed to draw observation serie")
                        .label(sv.to_string().to_owned() + "(OBS)")
                        .legend(|(x, y)| Cross::new((x, y), 4, color.clone()));
                }
                /*
                ...
                for (sv, color) in &cmap {
                        ...
                        .filter_map(|(index, (epoch, vehicules))| {
                            if vehicules.contains(&sv) {
                                if index == 0 {
                                    ...
                                    Some((
                                        epoch.to_utc_seconds() - nav_dates.0,
                                        sv.prn as f64 + 0.5,
                                    ))
                                }
                            } else {
                    ...
                    chart
                        .draw_series(
                            data.iter()
                                .map(|point| TriangleMarker::new(*point, 4, color.clone())),
                        )
                        .expect("failed to draw navigation serie")
                        .label(sv.to_string().to_owned() + "(EPH)")
                        .legend(|(x, y)| TriangleMarker::new((x, y), 4, color.clone()));
                }
                chart
        ...
        for (sv, color) in cmap.iter() {
            ...
            chart
                .draw_series(data.iter().map(|(x, y)| {
                    let p = (*x, y + constell_offsets.get(&sv.constellation).unwrap());
                    match sv.constellation {
                        Constellation::GPS | Constellation::Geo | Constellation::SBAS(_) => {
                            Circle::new(p, 4, Into::<ShapeStyle>::into(&color).filled()).into_dyn()
                        },
                        Constellation::Glonass => {
                            Cross::new(p, 4, Into::<ShapeStyle>::into(&color).filled()).into_dyn()
                        },
                        _ => TriangleMarker::new(p, 4, Into::<ShapeStyle>::into(&color).filled())
                            .into_dyn(),
                    }
                }))
                .expect("failed to draw serie")
                .label(sv.to_string())
                .legend(|(x, y)| match sv.constellation {
                    Constellation::GPS | Constellation::Geo | Constellation::SBAS(_) => {
                        Circle::new((x, y), 4, Into::<ShapeStyle>::into(color.clone()).filled())
                            .into_dyn()
                    },
                    Constellation::Glonass => {
                        Cross::new((x, y), 4, Into::<ShapeStyle>::into(color.clone()).filled())
                            .into_dyn()
                    },
