...
## RINEX Quality Check (QC)

Some QC capacities are under development.
Refer to [dedicated page](doc/qc.md)
...
The tool expects one local RINEX file given with `--fp`.  
...
Basic Identification comprises Epoch enumeration, or vehicule enumeration for instance.
It is possible to perform RINEX identification on a previously filtered or resampled RINEX.
Plotting is not possible, but enhanced JSON exists.

For example, identify encountered epoch and make it readable:
...
For example, also identify encountered vehicules:
