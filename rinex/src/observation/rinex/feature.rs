...
impl Rinex {
    ...
    pub fn carrier_iter(&self) -> Box<dyn Iterator<Item = (ObsKey, Carrier)> + '_> {
        Box::new(
            self.signal_observation_iter()
                .filter_map(|(k, sig)| {
                    if let Ok(carrier) =
    ...
    pub fn signal_code_iter(&self) -> Box<dyn Iterator<Item = (ObsKey, String)> + '_> {
        Box::new(
            self.signal_observation_iter()
                .filter_map(|(k, sig)| {
    ...
    pub fn signal_ok_iter(&self) -> Box<dyn Iterator<Item = (ObsKey, &SignalObservation)> + '_> {
        Box::new(self.signal_observation_iter().filter_map(|(k, sig)| {
            if !k.flag.is_ok() {
                Some((*k, sig))
            } else {
                None
    ...
    pub fn epoch_anomalies(&self) -> Box<dyn Iterator<Item = ObsKey> + '_> {
        Box::new(self.signal_observation_iter().filter_map(|(k, _)| {
            if !k.flag.is_ok() {
                Some(*k)
            } else {
                None
