                        ...
                        if let Some(dot) = items[5].find(".") {
                            ...
                            if let Ok(ss) = u8::from_str_radix(&items[5][..dot].trim(), 10) {
                                if let Ok(mut ns) = u32::from_str_radix(&items[5][dot+1..].trim(), 10) {
                                    ...
                                    let mut e: Epoch = match Epoch::maybe_from_gregorian_utc(y, m, d, hh, mm, ss, ns) {
                                        Ok(e) => e,
                                        Err(_) => {
                                            /*
                                             * UTC timestamp parsing failed,
                                             * probably due to YY encoded on two digits.
                                             */
                                            Epoch::maybe_from_gregorian_utc(y + 2000, m, d, hh, mm, ss, ns)?
                                        },
                                    };
                            ...
                            if let Ok(ss) = u8::from_str_radix(&items[5].trim(), 10) {
                                if let Ok(e) = Epoch::maybe_from_gregorian_utc(y, m, d, hh, mm, ss, 0) {
                                    Ok((e, EpochFlag::Ok))
                                } else {
                                    /*
                                     * UTC timestamp parsing in failure,
                                     * probably due to YY encoded on two digits.
                                     */
                                    Ok((Epoch::maybe_from_gregorian_utc(y + 2000, m, d, hh, mm, ss, 0)?, EpochFlag::Ok))
                                }
                            } else {
                                Err(Error::SecondsError)
                            }
...
mod test {
    ...
    fn test_nav_v2() {
        ...
        /*let (e, f) = parse("21  1  1 16 15  0.0");
        assert_eq!(e.is_ok(), true);
        let e = e.unwrap();
        let (y, m, d, hh, mm, ss, ns) = e.to_gregorian_utc();
        assert_eq!(y, 2021);
