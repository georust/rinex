...
mod test {
    ...
    fn test_min() {
        ...
        assert!(rinex.is_ok(), "Failed to parse RINEX file \"{}\"", "OBS/V3/DUTH0630.22O");
        let rinex = rinex
            .unwrap();
        ...
        assert!(g01_min.is_some(), "dataset.min() is missing results for vehicule G01");
        let g01_min = g01_min.unwrap();
        let g01_c1c_min = g01_min.get(&Observable::from_str("C1C").unwrap());
        assert!(g01_c1c_min.is_some(), "dataset.min() is missing results for C1C observable");
        ...
        assert!(min_s1c_observations.is_some(), "dataset.min() is missing results for S1C observable");
    ...
    fn test_max() {
        ...
        assert!(g01_max.is_some(), "dataset.max() is missing results for vehicule G01");
        let g01_max = g01_max.unwrap();

        let g01_c1c_max = g01_max
            .get(&Observable::from_str("C1C").unwrap());
        assert!(g01_c1c_max.is_some(), "dataset.max() is missing results for C1C observable");
        ...
        assert!(max_s1c_observations.is_some(), "dataset.max() is missing results for S1C observable");
